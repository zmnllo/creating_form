...
$query = "SELECT p.id_programme, p.nom_programme, 
          ...
          ORDER BY p.id_programme, o.id_objectif, e.id_exercice";
...
foreach ($programmes as $prog) {
    ...
    if (!isset($programmeData[$id_objectif]['programmes'][$id_programme])) {
        $programmeData[$id_objectif]['programmes'][$id_programme] = [
            ...
            'objectifs' => [], // Initialiser un tableau pour les objectifs
            'exercices' => []
        ];
    }
    
    // Ajouter l'objectif à la liste des objectifs du programme
    if (!empty($prog['nom_objectif'])) {
        $programmeData[$id_objectif]['programmes'][$id_programme]['objectifs'][] = [
            'nom_objectif' => $prog['nom_objectif']
        ];
    }
    

    // Ajoute les exercices (max 3)
    if (!empty($prog['nom_exercice']) && count($programmeData[$id_objectif]['programmes'][$id_programme]['exercices']) < 3) {
        $programmeData[$id_objectif]['programmes'][$id_programme]['exercices'][] = [
            'id_exercice' => $prog['id_exercice'],
            'nom_exercice' => !empty($prog['nom_exercice']) ? htmlspecialchars($prog['nom_exercice']) : 'Exercice inconnu',
...
<body>
    ...
    <?php foreach ($programmeData as $id_objectif => $objectif) : ?>
        <h1>📌 Objectifs : 
            <?php 
        if (!empty($programme['objectifs']) && is_array($programme['objectifs'])) {
            echo htmlspecialchars(implode(', ', array_unique(array_column($programme['objectifs'], 'nom_objectif'))));
        } else {
            echo "Aucun objectif défini";
        }
        ?>
</h1>
...
<?php if (empty($objectif['programmes'])) : ?>
    ...
    <?php else : ?>
        <?php foreach ($objectif['programmes'] as $id_programme => $programme) : ?>
            <h2 class="program-title">📌 <?= htmlspecialchars($programme['nom_programme']) ?></h2>
